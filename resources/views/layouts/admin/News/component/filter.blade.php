<div class="mb-3 row">
    <!-- Split dropstart button -->
                   ...
                   <ul class="dropdown-menu" style="min-width: 100vh">
                       <li>
                           <div class="container p-3">
                               <h5>Bộ lọc tin tức</h5>
                               <form class="row gy-1 mt-2" action="{{ route('news.filter') }}" method="get">
                                   @csrf
                                   ...
                                   {{-- <div class="col-md-6 col-lg-6">
                                       <div class="form-floating">
                                           <select
                                           class="form-select" autocomplete="brand" name="brand" id="brand">
                                               <option value="">Chọn</option>
                                               @foreach ($brands as $brand)
                                               <option 
                                               value="{{$brand->name}}" @if (request()->get('brand') == $brand->name) selected @endif>{{$brand->name}}</option>
                                   ...
                                   <div class="col-md-12 col-lg-12">
                                       ...
                                       <a href="{{ route('news.index') }}" class="btn btn btn-danger">Bỏ lọc</a>
                                   </div>
                               </form>
                           </div>
                       </li>
       ...
       <div class="col-lg-4">
           <form class="d-flex justify-content-end" action="{{ route('news.filter') }}" method="get">
               @csrf
               <div class="col-md-12">
                    <div class="input-group">
                        <input type="text" class="form-control border-0 " name="search" value="{{request()->get('search')}}" placeholder="Tìm kiếm tin tức">
